...
export const QRCodeDialog = ({
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(qrPaymentUrl).then(() => {
      toast({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-yellow-50 border-yellow-200 shadow-amber-200/30 shadow-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-amber-700">Maak een QR code voor een fooi betaalverzoek</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative mt-1">
              ...
              <Input id="amount" type="number" value={amount === 0 ? '' : amount} onChange={e => setAmount(parseFloat(e.target.value) || 0)} placeholder="0.00" min="0" step="0.50" className="pl-8 bg-white border-amber-200 focus:border-amber-400 focus:ring-amber-400" />
            </div>
          </div>
          
          <div>
            <Label htmlFor="note" className="text-amber-900">Notitie (optioneel)</Label>
            <Textarea id="note" value={note} onChange={e => setNote(e.target.value)} placeholder="Bijvoorbeeld: Dank voor de muziek!" className="mt-1 bg-white border-amber-200 focus:border-amber-400 focus:ring-amber-400" rows={2} />
          </div>
          ...
          <div className="flex justify-center py-4">
            <div className="bg-white p-4 rounded-lg shadow-md">
              <div id="qr-code-svg">
                <QRCodeSVG value={qrPaymentUrl} size={200} bgColor="#FFFFFF" fgColor="#000000" level="H" includeMargin={true} />
              </div>
              <canvas id="qr-code-canvas" style={{
              display: 'none'
            }} />
              <p className="text-center mt-2 text-amber-900 font-medium">
                {formatCurrency(amount)}
              </p>
              {note && <p className="text-center text-sm text-amber-700 mt-1">
                  {note}
                </p>}
            </div>
          </div>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          <Button variant="outline" className="flex-1 border-amber-300 text-amber-800 hover:bg-amber-100" onClick={handleCopyLink}>
            <Copy className="mr-2 h-4 w-4" />
            Kopieer Link
          </Button>
          <Button variant="outline" className="flex-1 border-amber-300 text-amber-800 hover:bg-amber-100" onClick={handleDownload}>
            <Download className="mr-2 h-4 w-4" />
            Download
          </Button>
          <Button variant="goldGradient" className="flex-1" onClick={handleShare}>
